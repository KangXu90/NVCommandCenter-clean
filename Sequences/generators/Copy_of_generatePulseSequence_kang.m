function [PSeq,fn] = generatePulseSequence_kang(pi_2, pi, N_pi, pi_2_phases, pi_phaseBlock,corrTime,script)
...
function[Channels] = generateChannels(pi_2,pi,N_pi,pi_2_phases, pi_phaseBlock,corrTime)
    ...
    delay = 0;
    Channels(2).addRise();
    Channels(2).setRiseParams(2,runningTime + delayAOM_MW + delay ,laserPulse,'',0,0,0,1);
    runningTime = runningTime + delayAOM_MW + delay;%delay between end of pulse sequence and read out
    %Configure Counter
    Channels(1).addRise();
    Channels(1).setRiseParams(1,runningTime+delayDAQ_AOM,counterGate,'Counter',0,0,0,1);
    ...
    Channels(2).addRise();
    Channels(2).setRiseParams(3,runningTime + delayAOM_MW + delay ,laserPulse/2,'',0,0,0,1);
    runningTime = runningTime + delayAOM_MW + delay;%delay between end of pulse sequence and read out
    %Configure Counter
    Channels(1).addRise();
