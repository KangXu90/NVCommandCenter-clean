function [PSeq,fn] = generatePulseSequence_RF_kang(pi_2, pi, N_pi, pi_2_phases, pi_phaseBlock,corrTime,script,ref,RiseAmp)
    ...
    PSeq = PulseSequence(Channels,[],Sweeps,1,num2str(N_pi));
...
% Initialize channels1
    ...
    % add the first RF pulse before first segment
        Channels(4).addRise();

    Channels(4).setRiseParams(1,runningTime,RF_duration,'RF',1,0,0,1);

    runningTime = runningTime + RF_duration + delayMW_AOM;
    ...
    for i = 1:Channels(3).NumberOfRises
        ...
        elseif i == 2
            Channels(3).setRiseParams(i,runningTime + corrTime,pi,'sweep',RiseAmp,phases(i),0,1);
            runningTime = runningTime + pi + PulseSpacing + corrTime;
        else
            Channels(3).setRiseParams(i,runningTime + corrTime*2,pi,'sweep',RiseAmp,phases(i),0,2);
    ...
    % add the first RF pulse before first segment
        ...
        Channels(4).setRiseParams(2,runningTime-RF_duration-delayMW_AOM,RF_duration,'RF',1,0,0,1);
    ...
    Channels(4).setRiseParams(3,runningTime,RF_duration,'RF',1,0,0,1);
    ...
    runningTime = runningTime + RF_duration + delayMW_AOM;
       ...
       runningTime = runningTime + correlationtime;
        Channels(4).setRiseParams(4,runningTime-RF_duration-delayMW_AOM,RF_duration,'RF',1,0,0,1);
